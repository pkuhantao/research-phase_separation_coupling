...
public:
    ...
    void interpMu(const vector<vector<vector<double> > > &oppMu); // given mu in opposite grids, interpolate the ghost cells for mu in current grids
    void calcMu_dw(const double ctPsi); // calculate mu from double well potential, given psi at center
    void calcMu_sd(); // calculate mu from simple diffusion model, where mu=psi
    void updatePsi(const double ctMu); // update psi from mu, given mu at center
    ...
    NormalGrids3D *grids3D; // 3D normal grids
